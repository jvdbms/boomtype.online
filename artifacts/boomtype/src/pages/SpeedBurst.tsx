...
export default function SpeedBurst() {
  ...
  const [highScore] = useState(() => parseInt(localStorage.getItem(HIGH_SCORE_KEY) || "0", 10));
    ...
    tickRef.current = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) {
          ...
          setScore(s => {
            setFinalScore(s);
            if (s > parseInt(localStorage.getItem(HIGH_SCORE_KEY) || "0", 10)) {
          ...
          {gameState === "idle" && (
            <div className="absolute inset-0 flex items-center justify-center bg-card/80 backdrop-blur-sm z-20">
              <div className="text-center">
                ...
                <p className="text-muted-foreground mb-2 max-w-xs">Bubbles appear with words. Type them to pop before they vanish. Build combos for multipliers!</p>
                {highScore > 0 && <p className="text-primary font-bold mb-4 flex items-center justify-center gap-1"><Trophy className="w-4 h-4" />Best: {highScore} pts</p>}
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-card/90 backdrop-blur-sm z-20">
              <div className="text-center">
                ...
                {finalScore >= highScore && finalScore > 0 && (
                  <p className="text-yellow-400 font-bold mb-3 text-sm">🏆 New High Score!</p>
                )}
                {finalScore < highScore && <p className="text-muted-foreground mb-3 text-sm">Best: {highScore} pts</p>}
